...
import 'package:bit_math/screens/playing_page/stages/actor/condition_decoration/condition_decoration.dart';
...
enum BitmanHelmet{
  none,strawHat,cape,hardHat,helmet,armor,ring
}
...
class Bitman extends SpriteAnimationGroupComponent<BitmanStatus> with HasGameRef<BitmanMath>,CollisionCallbacks,TapCallbacks{
  ...
  FutureOr<void> onLoad() {
    ...
    if(screenStatus==ScreenStatus.playing){
    add(CircleHitbox());
    ...
    }else if(screenStatus==ScreenStatus.result){
      current = BitmanStatus.dead;
    }
    return super.onLoad();
  }
  ...
  Future<void> onCollision(Set<Vector2> intersectionPoints, PositionComponent other)async {
    // only in playingpage,collision occur
    if(screenStatus==ScreenStatus.playing){
      if(other is Ridable){
        ...
        if(fromBelow.dot(collisionNormal)>0.9){
          if(other is InputBlockBorder){
            ...
            if(!inputting && !solved){
              ...
              // avoid too many input by one tap
              Future.delayed(const Duration(milliseconds: 800))
              .whenComplete(() => inputting =false);
            }
    ...
    if(other is Spring){
      ...
      other.add(ColorEffect(Colors.red, EffectController(
        duration: 0.3
  ...
  void update(double dt) {
    // only plaingpage update occur
    if(screenStatus==ScreenStatus.playing){
    //gameover logic
    if(position.y>16*20
    //game.canvasSize.y
    ) findParent<PlayingPage>()!.state.fallDown = true;
  ...
  void onTapDown(TapDownEvent event) {
    if(screenStatus==ScreenStatus.home){
