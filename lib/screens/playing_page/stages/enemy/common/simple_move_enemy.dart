...
class SimpleMoveEnemy extends SpriteGroupComponent<SimpleMoveEnemyStatus> with StageBlock,EnemyCondition,HasGameRef<BitmanMath> implements Enemy,StageObject,HasPathEffect{
 SimpleMoveEnemy(
  ...
  FutureOr<void> onLoad() {
    ...
    isPressable = true;
     ...
     case SimpleMoveEnemyStatus.slime:{
      ...
      add(ColorEffect(
        colors[random.nextInt(colors.length)], EffectController(
          duration: 2.0,
          alternate: true,
          infinite: true
        ),opacityFrom: 0.7,opacityTo: 0.2));
    life=1;
    power=2;
    isPressable = true;
     }
  ...
  void update(double dt) {
    scrollMove(dt);
    if(status==SimpleMoveEnemyStatus.aligator){
      final level = game.gameState.level;
